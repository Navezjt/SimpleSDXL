## SimpleSDXL首屏
<div align=center><img src="https://github.com/metercai/SimpleSDXL/assets/5652458/11be4706-7d73-47b3-a880-4ea895e44464"></div>
...
- [2023.11.30] 重要更新：新增图片参数工具箱，实现图片浏览过程中对生成参数的及时查看，数据源来自于图片目录的 `log.html` ，可以与主线版本无缝衔接。这次更新给后续的生成参数嵌入图片，以及基于嵌入参数图片的再生成打下了基础。虽然非常赞同和支持Fooocus作者强调的隐私保护策略，图片生成不会将标识和参数嵌入图片。但在实际场景中，嵌入参数的图片有很多用处。为此，SimpleSDXL将会提供快捷手动工具支持个别图片的参数嵌入。同时保持生成的图片默认不嵌入标识和参数的原功能不变。敬请期待。
