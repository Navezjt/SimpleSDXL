...
## 🔔 更新 / Update
- [2023.11.30] 重要更新：新增图片参数工具箱，实现图片浏览过程中对生成参数的及时查看，数据源来自于图片目录的 `log.html` ，可以与主线版本无缝衔接。这次更新给后续的生成参数嵌入图片，以及基于嵌入参数图片的再生成打下了基础。虽然非常赞同和支持Fooocus作者强调的隐私保护策略，图片生成不会将标识和参数嵌入图片。但在实际场景中，嵌入参数的图片有多的价值。为此，SimpleSDXL将会提供额外的手动工具来支持这种场景，默认生成图片仍然不会嵌入标识和参数，敬请期待。
