    ...
    with gr.Row():
                ...
                with gr.Tabs():
                    with gr.TabItem(label='强化与变换') as uov_tab:
                        with gr.Row():
                            ...
                            with gr.Column():
                                uov_method = gr.Radio(label='强化(Upscale)与变换(Vary)：', choices=flags.uov_list, value=flags.disabled)
                                ...
                                with gr.Column():
                                    ...
                                    with gr.Column(visible=False) as ad_col:
                                        with gr.Row():
                                            ...
                                            ip_stop = gr.Slider(label='停在', minimum=0.0, maximum=1.0, step=0.001, value=default_end)
                    ...
                    with gr.TabItem(label='修补与融合（测试）') as inpaint_tab:
                        ...
                        gr.HTML('向外绘制扩展 (<a href="https://github.com/lllyasviel/Fooocus/discussions/414" target="_blank">\U0001F4D4 参考文档</a>):')
        ...
        with gr.Column(scale=1, visible=False) as advanced_column:
            with gr.Tab(label='设置'):
                performance_selection = gr.Radio(label='Speed速度优先/Quality质量优先）', choices=['Speed', 'Quality'], value='Speed')
            ...
            with gr.Tab(label='模型'):
                ...
                with gr.Accordion(label='LoRA局部调整模型', open=True):
                    lora_ctrls = []
                    for i in range(5):
                        with gr.Row():
            ...
            with gr.Tab(label='高级'):
                ...
                with gr.Column(visible=False) as dev_tools:
                    ...
                    with gr.Tab(label='图像控制'):
                        debugging_cn_preprocessor = gr.Checkbox(label='调试预处理器', value=False)

                        mixing_image_prompt_and_vary_upscale = gr.Checkbox(label='混合图像提示和变换/增强',
                                                                           value=False)
