    ...
    with gr.Row():
        ...
        def toggle_engine(x, aspect_ratios, sd3_aspect_ratios, hydit_aspect_ratios):
            if x==flags.backend_engines[2]:
                results = [gr.update(value=False, interactive=False), gr.update(visible=False), gr.update(value=4.5), gr.update(visible=False), gr.update(visible=True), gr.update(visible=False), sd3_aspect_ratios, gr.update(), 'dpmpp_2m', 'sgm_uniform', 28, 'sd3_medium_incl_clips_t5xxlfp8.safetensors'] + [gr.update()] * 16
                comfyd.start(args_comfyd)
            elif x==flags.backend_engines[1]:
                results = [gr.update(value=False, interactive=False), gr.update(choices=flags.Performance.list()[:2], visible=True), gr.update(value=6), gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), hydit_aspect_ratios,  gr.update(value=[]), gr.update(choices=hydit_task.SAMPLERS, value=hydit_task.default_sampler)] + [gr.update(interactive=False)] * 19
            else:
                results = [gr.update(interactive=True), gr.update(choices=flags.Performance.list(), visible=True), gr.update(value=modules.config.default_cfg_scale), gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), aspect_ratios, gr.update(value=copy.deepcopy(modules.config.default_styles)), gr.update(choices=flags.sampler_list, value=modules.config.default_sampler), gr.update(choices=flags.scheduler_list, value=modules.config.default_scheduler), -1, modules.config.default_base_model_name] + [gr.update(interactive=True)] * 16
            return results

        current_aspect_ratios = gr.Textbox(value='', visible=False)
        backend_selection.change(toggle_engine, inputs=[backend_selection, aspect_ratios_selection, sd3_aspect_ratios_selection, hydit_aspect_ratios_selection], outputs = [input_image_checkbox, performance_selection, guidance_scale, aspect_ratios_selection, sd3_aspect_ratios_selection, hydit_aspect_ratios_selection, current_aspect_ratios, style_selections, sampler_name, scheduler_name, overwrite_step, base_model, refiner_model] + lora_ctrls, queue=False, show_progress=False).then(lambda x: None, inputs=current_aspect_ratios, queue=False, show_progress=False, _js='(x)=>{refresh_aspect_ratios_label(x);}')
